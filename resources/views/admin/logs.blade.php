        ...
        <form id="activityFilterForm" class="activity-filter mt-3" method="GET" action="{{ route('admin.logs.index') }}">
            ...
            <div class="filter-buttons" style="display:flex;gap:0.3rem;flex-wrap:wrap">
                ...
                @if($startDate || $endDate || $search || $type !== 'all')
                    <a href="{{ route('admin.logs.index') }}" class="btn btn-sm btn-outline-secondary" style="height:44px;border-radius:12px;padding:0 1rem;white-space:nowrap;text-decoration:none;display:flex;align-items:center;flex:1;justify-content:center">✕ Clear</a>
                @endif
            </div>
        </form>
    </section>
