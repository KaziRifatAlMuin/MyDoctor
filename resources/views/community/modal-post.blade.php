            ...
            <div style="display: flex; gap: 12px; cursor: {{ $isAnonymous ? 'default' : 'pointer' }};" @if(!$isAnonymous) onclick="showUserModal({{ $post->user->id }})" @endif>
                ...
                <div>
                    ...
                    <div style="display: flex; align-items: center; gap: 12px; font-size: 12px; color: #65676b; margin: 0; padding: 0;">
                        ...
                        @endif
                        @if($post->disease)
                            <a href="{{ route('public.diseases.show', $post->disease) }}" style="background: #e7f3ff; color: #1877f2; padding: 4px 12px; border-radius: 4px; font-weight: 500; font-size: 12px; display: inline-flex; align-items: center; gap: 4px; text-decoration: none;">
